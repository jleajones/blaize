...
describe('Logger', () => {
  ...
  describe('Performance characteristics', () => {
    test('filtered logs have minimal overhead', () => {
      const logger = new Logger({
        level: 'error',
        transport,
      ...
      // Measure time for 10000 filtered debug logs
      ...
      // Should be very fast (< 10ms for 10k filtered logs)
      // This ensures zero-overhead filtering is working
      expect(duration).toBeLessThan(10);
      expect(transport.logs).toHaveLength(0);
    });

    test('enabled logs process efficiently', () => {
      const logger = new Logger({
        level: 'info',
        transport,
        redactKeys: ['password'],
      ...
      // Measure time for 1000 enabled logs
      const start = performance.now();
      for (let i = 0; i < 1000; i++) {
        logger.info('Operation', { userId: i, action: 'test' });
      }
      const end = performance.now();
      const duration = end - start;

      // Should be reasonably fast (< 100ms for 1k logs)
      expect(duration).toBeLessThan(100);
      expect(transport.logs).toHaveLength(1000);
    });
  });
});
