...
describe('Origin Validator', () => {
  ...
  describe('Performance', () => {
    test('should handle 1000 string validations efficiently with caching', async () => {
      setOriginCache(60000, 1000);

      const origins: string[] = [];

      // Generate 1000 unique origins
      for (let i = 0; i < 1000; i++) {
        origins.push(`https://origin${i}.com`);
      }

      const start = Date.now();

      // Validate all origins against wildcard (cacheable)
      const results = await Promise.all(origins.map(origin => validateOrigin(origin, '*')));

      const duration = Date.now() - start;

      // Should complete quickly (< 100ms for 1000 validations)
      expect(duration).toBeLessThan(100);

      // Check results
      const allowedCount = results.filter(r => r === true).length;
      expect(allowedCount).toBe(1000); // All should be allowed by wildcard

      // Cache should be at max size
      expect(getCacheStats().size).toBe(1000);

      // Validate again - should be instant from cache
      const cacheStart = Date.now();
      await Promise.all(origins.map(origin => validateOrigin(origin, '*')));
    ...
    test('should handle concurrent validations with string origins correctly', async () => {
      ...
      // Make 10 concurrent calls for the same origin
      const promises = Array(10)
        .fill(null)
        .map(() => validateOrigin('https://example.com', configuredOrigin));
      ...
      // Should only create one cache entry
      expect(getCacheStats().size).toBe(1);
    });

    test('function validators should be called every time in concurrent scenarios', async () => {
      const validator = vi.fn(async () => {
        await new Promise(resolve => setTimeout(resolve, 10));
        return true;
      });

      // Make 10 concurrent calls for the same origin with function validator
      const promises = Array(10)
        .fill(null)
        .map(() => validateOrigin('https://example.com', validator));
      ...
      // Validator should be called 10 times (no caching for functions)
