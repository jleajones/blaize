...
> Types flow from your Zod schemas through your handlers to your client — no code generation, no manual syncing, no runtime overhead.
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   Zod Schema    │────▶│  Route Factory  │────▶│  Route Export   │
│   (Runtime)     │     │   route.get()   │     │   getUser       │
└─────────────────┘     └─────────────────┘     └─────────────────┘
        │                       │                       │
        ▼                       ▼                       ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  z.infer<T>     │     │ InferContext<>  │     │ typeof routes   │
│  Params, Body   │     │ State, Services │     │   Registry      │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                        │
                                                        ▼
                                                ┌─────────────────┐
...
**The key insight:** TypeScript infers types at each step without generating any code. The types exist only at compile time — zero runtime overhead.
...
export const getUser = route.get({
  ...
  handler: async (ctx, params) => {
    ...
    // ctx.request.query.include is typed as string[] | undefined ✅
...
export const GET = route.get({
  schema: {
    params: z.object({
      // Input: string from URL → Output: number after transform
...
│                    CLIENT (Browser/Node)                         │
├─────────────────────────────────────────────────────────────────┤
│  file() schema → ClientFilesType<F>                             │
│  - Extracts schema shape                                         │
│  - Maps to File/Blob types                                       │
│  - Handles optional/array modifiers                              │
└─────────────────────────────────────────────────────────────────┘
                              ↓
...
│                    SERVER (BlaizeJS)                             │
├─────────────────────────────────────────────────────────────────┤
│  Multipart parser → { fields, files }                            │
│  file() schema → z.ZodType<UploadedFile>                        │
│  - Validates MIME type                                           │
│  - Validates file size                                           │
│  - Custom refinements                                            │
│  ↓                                                               │
│  Handler receives typed UploadedFile                             │
│  - originalname: string                                          │
│  - mimetype: string                                              │
│  - size: number                                                  │
│  - buffer: Buffer                                                │
...
// - AppContext['state']    → Combined state from all middleware
// - AppContext['services'] → Combined services from all plugins
```
...
Don't ignore the return value — types will be lost:

```typescript
const server = Blaize.createServer({ port: 3000, routesDir: './src/routes' });
server.use(authMiddleware); // ❌ Return value ignored!
server.register(databasePlugin); // ❌ Return value ignored!

type AppContext = InferContext<typeof server>;
// AppContext['state'] = {}       ← Missing user!
// AppContext['services'] = {}    ← Missing db!
```
...
export const GET = route.get({
  handler: async (ctx, params) => {
    ctx.state.user; // ✅ Typed as User
    ctx.services.db; // ✅ Typed as Database
  },
});
```
...
```
src/
├── app.ts              ← Server created, types inferred
│   └── route exported  ← Factory carries types
├── routes/
│   ├── users.ts        ← Imports route, gets types
│   └── posts.ts        ← Imports route, gets types
```
...
When you add middleware to the server, types compose automatically — but remember to capture return values:
...
import { PATCH as updateAvatar } from './routes/users/[userId]/avatar';
import { POST as login } from './routes/auth/login';

// Export with meaningful names — these become method names
export const routes = {
  getUser,
  listUsers,
  createUser,
...
// ✅ Files typed from schema - client auto-detects and uses multipart
const file = document.querySelector('input[type="file"]').files[0];
...
| ----------------------- | ----------------------------------------------- |
| Schema → Handler params | `params.userId` typed from `schema.params`      |
| Schema → Handler body   | `ctx.request.body` typed from `schema.body`     |
| Schema → Handler files  | `ctx.request.files` typed from `schema.files`   |
| Schema → Handler return | Return type checked against `schema.response`   |
| Middleware → Context    | `ctx.state.user` typed from middleware generics |
| Route export → Client   | Client methods typed from route exports         |
...
1. **Zod schemas are the source of truth** — They define runtime validation AND TypeScript types
2. **`InferContext` captures composed types** — All middleware and plugin contributions combined
3. **Capture return values** — `.use()` and `.register()` return new typed servers; always capture or chain
4. **Route factory carries types** — Creates the bridge from server to route files
5. **Export names become client methods** — You control the client API
6. **No code generation** — Everything is TypeScript inference at compile time
7. **Transforms work** — Zod `.transform()` and `.coerce` flow through correctly
8. **File schemas use `file()` helper** — Provides size/MIME validation with automatic type inference
9. **Client-server type mapping** — Same schema creates `File`/`Blob` types on client, `UploadedFile` on server
10. **Validation at the boundary** — Files validated before handler execution, maintaining type safety

---

## Related Pages

- **[Context](/docs/architecture/context)** — The object that carries state and services
- **[Middleware](/docs/architecture/middleware)** — How middleware adds to context types
- **[Plugins](/docs/architecture/plugins)** — How plugins inject services
- **[Routing](/docs/architecture/router)** — File-based routing and route exports
- **[File Uploads](/docs/guides/file-uploads)** — Complete guide with storage integration