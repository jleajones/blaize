...
├──────────────────────────────────────────────────────────────────────────┤
│                         Event Bus                                       │
│                  (in-memory with distributed adapter)                  │
├──────────────────────────────────────────────────────────────────────────┤
│                         Plugin System                                   │
│         (lifecycle: register → initialize → start → stop)              │
├──────────────────────────────────────────────────────────────────────────┤
│                      Middleware Pipeline                                │
│              (composable, typed, onion-style execution)                │
├──────────────────────────────────────────────────────────────────────────┤
│                       File-Based Router                                 │
│     (auto-discovery, dynamic parameters, multipart parsing, exports)   │
├──────────────────────────────────────────────────────────────────────────┤
│                    Context (AsyncLocalStorage)                          │
│         (request, response, state, services, correlation ID)          │
├──────────────────────────────────────────────────────────────────────────┤
│                        Error Boundary                                   │
│          (catches all errors, formats responses, adds IDs)             │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| **Router**         | URL → Handler mapping, parameter extraction, **multipart parsing** | `router/file-router.ts`, **`upload/multipart-parser.ts`** |
...
│                                                                          │
│  ┌─────────┐    ┌─────────────┐    ┌────────────┐    ┌───────────────┐  │
│  │ Request │───▶│   Context   │───▶│   Global   │───▶│    Route      │  │
│  │ Arrives │    │  Creation   │    │ Middleware │    │   Matching    │  │
│  └─────────┘    └─────────────┘    └────────────┘    └───────────────┘  │
│                                                              │           │
│                                                              ▼           │
│  ┌─────────┐    ┌─────────────┐    ┌────────────┐    ┌───────────────┐  │
│  │Response │◀────│   Error     │◀────│  Handler   │◀────│    Route      │  │
│  │  Sent   │    │  Boundary   │    │ Execution  │    │  Middleware   │  │
│  └─────────┘    └─────────────┘    └────────────┘    └───────────────┘  │
│                                           │                              │
│                                           ▼                              │
│                                    ┌────────────┐                        │
...
Middleware executes in an "onion" pattern — each middleware wraps the next:
...
const validatedFiles = filesSchema.parse(ctx.request.files); // ← NEW
...
```
┌─────────────────────────────────────────────────────────────────┐
│ 1. Client detects File/Blob objects                            │
│    → Auto-switches to multipart/form-data                      │
│    → FormData created automatically                            │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 2. Server receives multipart request                           │
│    → Content-Type: multipart/form-data; boundary=...           │
│    → Multipart parser extracts files + fields                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 3. Request Validator validates files                           │
│    → File size checked (maxSize: '5MB')                        │
│    → MIME type validated (accept: ['image/*'])                 │
│    → Returns ValidationError if invalid                        │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│ 4. Handler receives typed files                                │
│    → ctx.request.files.avatar (typed as UploadedFile)          │
│    → ctx.request.body (typed from body schema)                 │
│    → Full type safety from schema to handler                   │
└─────────────────────────────────────────────────────────────────┘
```
...
- **Types:** `File`/`Blob` (client) → `UploadedFile` (server)
...
type InferParams<T> = T extends { schema: { params: infer P } } ? z.infer<P> : never;

type InferResponse<T> = T extends { schema: { response: infer R } } ? z.infer<R> : never;

type InferFiles<T> = T extends { schema: { files: infer F } } ? z.infer<F> : never;
...
> **📖 Deep Dive:** For detailed information about the event bus implementation, patterns, and best practices, see the [Event Bus Architecture Guide](./event-bus.md).
...
│               ▼                                   ▼                      │
│  ┌────────────────────────┐       ┌──────────────────────────────────┐  │
│  │  TypedEventBus         │       │   EventBusAdapter (Optional)     │  │
│  │  • Zod schema validation│       │   • Cross-server propagation    │  │
│  │  • Type-safe pub/sub   │       │   • Redis, RabbitMQ, etc.        │  │
│  │  • Compile-time safety │       │   • Self-filtering               │  │
│  └────────────────────────┘       └──────────────────────────────────┘  │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                          │
│  Features:                                                               │
│  • Self-filtering: Servers ignore their own events                       │
│  • Channel-based routing: Events published to pattern-specific channels  │
│  • Automatic reconnection: Subscriptions restored on reconnect           │
...
│       ▼                        ▼                       ▼                 │
│  ┌─────────┐    ┌────────────┐    ┌─────────────┐    ┌──────────┐      │
│  │ setup() │───▶│initialize()│───▶│onServerStart│───▶│terminate │      │
│  │         │    │            │    │     ()      │    │   ()     │      │
│  └─────────┘    └────────────┘    └─────────────┘    └──────────┘      │
│       │                │                  │                 │            │
│       │                │                  │                 │            │
│  • Config        • Connect to      • Start         • Disconnect         │
│    merge           external          background      from external      │
│  • Service         services          tasks           services           │
│    creation      • Database        • Subscribe      • Cleanup           │
│  • Middleware      init              to events        resources         │
│    register      • Resource        • Health         • Save state        │
│  • Event           allocation        checks                             │
│    schemas                                                               │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
│   │   ├── posts/
│   │   │   └── index.ts  → GET /users/:userId/posts
│   │   └── settings.ts   → GET /users/:userId/settings
...
│   │                                       │
│   ├──▶ stream.send('event', data) ──▶ Client receives
│   ├──▶ stream.send('event', data) ──▶ Client receives
│   ├──▶ stream.send('event', data) ──▶ Client receives
│   │                                       │
│   │  Client disconnects or network fails  │
│   ▼                                       │
│  stream.onClose() fires                   │
...
│      ▼                                                                   │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐             │
│  │  Enqueue Job │────▶│  Job Storage │◀────│    Worker    │             │
│  │  (priority)  │     │   (in-memory │     │   Process    │             │
│  └──────────────┘     │   or Redis)  │     └──────┬───────┘             │
│                       └──────────────┘            │                      │
│                                                   │                      │
│                                                   ▼                      │
...
```
              ...
              ▼              ▼              ▼
      ┌───────────┐  ┌───────────────┐  ┌────────────────────┐
      │ blaizejs  │  │ @blaizejs/    │  │ @blaizejs/         │
      │  (core)   │  │    client     │  │  testing-utils     │
      └─────┬─────┘  └───────────────┘  └────────────────────┘
            │
            │ Peer dependency
            │
...
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│ @blaizejs/      │  │ @blaizejs/      │  │ @blaizejs/      │
│  plugin-queue   │  │  plugin-cache   │  │  plugin-metrics │
└─────────────────┘  └─────────────────┘  └─────────────────┘
...
- **Unidirectional sufficient:** Most real-time needs are server → client
...
**Pattern:** For bidirectional communication, use POST requests for client → server and SSE for server → client. This pattern covers most real-time application needs.
...
- [Getting Started Guide](../docs/getting-started.md) — Build your first BlaizeJS app
- [Event Bus Architecture](./event-bus.md) — Deep dive into event-driven patterns
- [File Uploads Guide](../docs/guides/file-uploads.md) — Complete file upload examples
- [Core Package API](../packages/blaize-core/README.md) — Full API reference
- [Plugin Development Guide](../docs/guides/plugins.md) — Create custom plugins
- [Contributing Guide](../CONTRIBUTING.md) — Help improve BlaizeJS
