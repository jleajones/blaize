...
> The Context is the single object that flows through every request — carrying request data in, response methods out, and accumulating state along the way.
...
export const getUser = route.get({
  handler: async ({ ctx }) => {
    ...
    ctx.request.files; // File uploads (POST/PUT/PATCH)
...
> ⚠️ **Double Response Protection:** Attempting to send a response after one was already sent throws `ResponseSentError`. This prevents subtle bugs where headers are sent twice.

### 3. ctx.state — Per-request data
...
- ✅ Per-request (isolated between concurrent requests)
- ✅ Mutable (middleware adds properties)
- ✅ Typed (via middleware generics)
- ✅ Survives async boundaries (via AsyncLocalStorage)

### 4. ctx.services — Shared instances
...
- ✅ Shared (same instance across requests)
- ✅ Injected by plugins (via middleware)
- ✅ Typed (via middleware generics)
- ✅ Lifecycle-managed (plugins handle connect/disconnect)
...
- If it's **computed for each request** → `state`
- If it's a **shared connection/client** → `services`
...
5. Handler executes with fully-typed context
   handler({ ctx, params, logger, eventBus }) → response
        │
        ▼
...
```typescript
export const getData = route.get({
  handler: async ({ ctx }) => {
...
> 🔒 **Internal Note:** The `runWithContext()` and `getContext()` functions exist internally but are not exported. Context is passed explicitly to handlers for clarity and testability.
...
| `files`    | `Record<string, unknown>` | File uploads                 |
...
| `multipart/form-data`               | Files + fields | `ctx.request.files`, `ctx.request.body` |
...
Log request details with correlation:

```typescript
const requestLogMiddleware = createMiddleware<{ requestId: string }, {}>({
  name: 'request-log',
...
1. **Context is the single carrier** — Everything flows through `ctx`
2. **Four distinct parts** — `request` (in), `response` (out), `state` (per-request), `services` (shared)
3. **Typed via composition** — Middleware and plugins declare what they add
4. **Destructured in handlers** — Access via `{ ctx, params, logger, eventBus }`
5. **Testable** — `createTestContext` and `createRouteTestContext` make unit testing easy
6. **Protected** — Double responses throw errors, not silent failures
7. **Automatic propagation** — AsyncLocalStorage handles context across async boundaries

---

## Related pages

- **[Type System](/docs/architecture/type-system)** — How types flow from middleware to handlers
- **[Middleware](/docs/architecture/middleware)** — How middleware adds to state and services
- **[Plugins](/docs/architecture/plugins)** — How plugins provide services
- **[Router](/docs/architecture/router)** — How context is used in route handlers
- **[Testing Guide](/docs/guides/testing)** — Testing with `createTestContext`
